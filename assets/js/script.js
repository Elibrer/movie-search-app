$(function () {
  ...
  const summerMensURL = {
    shirt:
      ...
      summerMensShirtPID +
      "&lang=en-AU&store=AU&sizeSchema=AU&currency=AUD",
    shorts:
      "https://asos2.p.rapidapi.com/products/v3/detail?id=" +
      summerMensShortsPID +
      "&lang=en-AU&store=AU&sizeSchema=AU&currency=AUD",
  ...
  function init() {
    for (i = recentSearchArr.length - 1; i >= 0; i--) {
      var recentBtn = $(
        "<button class='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mb-1'></button>"
      );
  ...
  function cityTempSearch(city) {
    ...
    recentSearch =
      recentSearch.charAt(0).toUpperCase() +
      recentSearch.slice(1).toLowerCase();
  ...
  //     console.log("hello");
  //     for (i = 0; i < 3; i++) {
  //         let randomIndex = Math.floor(Math.random() * recentSearchArr.length);
  ...
  //             <a href="${clothesData.products[randomIndex].url}">
  //             <img src="https://${clothesData.media.images[0].imageUrl}" class="rounded-lg shadow-lg w-full scale-75">
  //             </a>
  //         `;
  ...
  function displayClothes(clothesData) {
    ...
    clothesContainer.setAttribute(
      "class",
      "mx-2.5 rounded-lg flex justify-center items-center w-full sm:w-2/12 bg-sky-900 text-white"
    );
    innerContainer.setAttribute(
      "class",
      "flex flex-wrap justify-center items-center mx-auto p-3"
    );
